...
export function makeFloorTexture(baseColor = "#C9A57B"): THREE.CanvasTexture {
  ...
  for (let i = 0; i < imgData.data.length; i += 4) {
    ...
    imgData.data[i] = clamp((imgData.data[i] ?? 0) + noise);
    imgData.data[i + 1] = clamp((imgData.data[i + 1] ?? 0) + noise);
    imgData.data[i + 2] = clamp((imgData.data[i + 2] ?? 0) + noise);
  }
...
export function makeWallTexture(baseColor = "#F0E5D2"): THREE.CanvasTexture {
  ...
  for (let i = 0; i < imgData.data.length; i += 4) {
    ...
    imgData.data[i] = clamp((imgData.data[i] ?? 0) + noise + warmTint);
    imgData.data[i + 1] = clamp((imgData.data[i + 1] ?? 0) + noise);
    imgData.data[i + 2] = clamp((imgData.data[i + 2] ?? 0) + noise - warmTint);
  }
  ctx.putImageData(imgData, 0, 0);
