...
export function ProjectsModal() {
  ...
  const switchTo = (id: string) => {
    const shouldSwitch = id !== currentProjectId;
    // Close first and defer the scene reset so React can unmount this
    // dialog before Demo apartment GLB work blocks the main thread.
    setOpen(false);
    if (!shouldSwitch) return;
    window.setTimeout(() => {
      useStore.getState().setCurrentProject(id);
    }, 0);
  };
