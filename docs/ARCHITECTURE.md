...
| Clerk                  | `/login` + `/signup` hosted Google/email                                                                 | `@clerk/nextjs` for the auth split. After Google, `/api/auth/clerk-callback` upserts the Prisma `User` and sets `furnishes_session`. Account APIs stay cookie-auth. |
